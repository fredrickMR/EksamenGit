...
function Service() {
    ...
    async function DeleteKunde(id) {
        try {
            const res = await fetch(`/api/kunde/delete/${id}`, {
                method: 'DELETE',
                headers
            });
            setKundeResult(await res.json());
    ...
    async function CreateBil() {
        try {
            const res = await fetch('/api/bil/post', {
                ...
                headers,
                body: JSON.stringify({
                    bil_registreringsnummer: bilReg,
                    modell_navn: bilModell,
                    eier_id: bilEier
                })
            });
            setBilResult(await res.json());
    ...
    async function DeleteBil(reg) {
        try {
            const res = await fetch(`/api/bil/delete/${reg}`, {
                method: 'DELETE',
                headers
            });
            setBilResult(await res.json());
                ...
                <div>
                    {items.map(k => (
                        <div key={k.kunde_id}>
                            <p>Id: {k.kunde_id}</p>
                            <p>Navn: {k.navn}</p>
                            <button onClick={() => DeleteKunde(k.kunde_id)}>
                                Delete
                            </button>
                        </div>
                    ))}
            ...
            <>
                <div>
                    <input
                        ...
                        value={bilReg}
                        onChange={e => setBilReg(e.target.value)}
                    />
                    <input
                        type="text"
                        placeholder="modell_navn"
                        value={bilModell}
                        onChange={e => setBilModell(e.target.value)}
                    />
                    <input
                        type="number"
                        placeholder="eier_id (kunde)"
                        value={bilEier}
                        onChange={e => setBilEier(e.target.value)}
                    />
                </div>
                ...
                <div>
                    {items.map(b => (
                        <div key={b.bil_registreringsnummer}>
                            ...
                            <button onClick={() => DeleteBil(b.bil_registreringsnummer)}>
                                Delete
                            </button>
                        </div>
                    ))}
        ...
        return (
            <>
                <div>
                    ...
                    <input
                        type="text"
                        placeholder="bil (reg.nr)"
                        value={serviceBil}
                        onChange={e => setServiceBil(e.target.value)}
                    />
                    <input
                        type="number"
                        placeholder="mekaniker_id"
                        value={serviceMekaniker}
                        onChange={e => setServiceMekaniker(e.target.value)}
                    />
                </div>
                ...
                <div>
                    {items.map(s => (
                        <div key={s.service_id}>
                            ...
                            <button onClick={() => DeleteService(s.service_id)}>
                                Delete
                            </button>
                        </div>
                    ))}
            ...
            <div>
                ...
                <div style={{ marginBottom: "1rem" }}>
                    <button
                        ...
                        style={{
                            fontWeight: activeTab === "kunde" ? "bold" : "normal"
                        }}
                    >
                        Kunder
                    </button>
                    ...
                    <button
                        ...
                        style={{
                            fontWeight: activeTab === "bil" ? "bold" : "normal"
                        }}
                    >
                        Biler
                    </button>
